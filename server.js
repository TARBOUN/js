...
const youtube = new YouTube("AIzaSyCJg81D8OpbnadFUroZirMOwu_ySgcR4X0"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyCJg81D8OpbnadFUroZirMOwu_ySgcR4X0"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("ready", () => {
  ...
  client.user.setActivity(`  ❤ جاستس فكرة والفكرة لا تموت `, { type: "STREAMING" });
});
...
/////  كود برفيكس 
client.on('message', wolf => {   if (wolf.content === "<@770673741913915403>") {     wolf.channel.send('My Prefix is `#`');  
...
if(!laith.guild.member(laith.author).hasPermission("MANAGE_CHANNELS")) return laith.channel.send("**للاسف ليس لديك صلاحيات كافيه**")
if(!laith.guild.member(laith.client.user).hasPermission("MANAGE_CHANNELS")) return laith.channel.send("**للاسف ليس لدي صلاحيات لتنفيذ هذ الامر**")
...
laith.channel.send("**تم تعين > SlowMode الى هذي القناة > : "+time+"**")
 ...
 /*
client.roles.cashe.get("")
const role = client.roles.cashe.get("")
...
client.on('message', message => {
  ...
  let args = message.content.split(" ").slice(1);
  /*
  client.roles.find("<@&771416655644327996><@&690672395765153805>")
  const role = client.roles.find("<@&771416655644327996><@&690672395765153805>")
  message.channel.send(`${role}`)
*/
  if(command == "embed")
/*message.channel.send("<@&771416655644327996><@&690672395765153805>")*/
var btrolie = new Discord.RichEmbed()
.setColor('RANDOM')
.setDescription(args.join("  "))
/*.setFooter("justiceGamingCommunity - Ticketing without clutter ", message.client.user.avatarURL)*/
...
client.on ('message', async (toxicc) => {
  ...
  switch (toxicc.content.split(' ') [0]){
    case prefix+'server':
      ...
      .addField ("Members: ", toxicc.guild.members.filter (member => !member.user.bot).size, true)
      .addField ("Bots: ", toxicc.guild.members.filter (member => member.user.bot).size, true)
...
////tickets witha reaction 

client.on('raw', packet => {
if(!['MESSAGE_REACTION_ADD', 'MESSAGE_REACTION_REMOVE'].includes(packet.t)) return;
if (packet.t == 'MESSAGE_REACTION_ADD') {
if(packet.d.message_id == '774312046929379378') { // ايدي المسج
...
let CH = message.guild.channels.find(r => r.id == '644106071257907200'); // ايدي الكاتوجري الي بتنحط تحتها التكتات
if(!CH) return;
channel.guild.createChannel(`ticket-${u.username}`,
{
  ...
  permissionOverwrites: [{
    ...
    id: '613107866798260323', // ايدي رتبه السبورت
    allow: ['SEND_MESSAGES','READ_MESSAGES','ATTACH_FILES','READ_MESSAGE_HISTORY']
},{
    id: '631328005762842624', // ايدي رتبه السبورت
    allow: ['SEND_MESSAGES','READ_MESSAGES','ATTACH_FILES','READ_MESSAGE_HISTORY']
},{
    id: '739987666774261840', // ايدي رتبه السبورت
    allow: ['SEND_MESSAGES','READ_MESSAGES','ATTACH_FILES','READ_MESSAGE_HISTORY']
},{
    id: '770644767070748683', // ايدي رتبه السبورت
    allow: ['SEND_MESSAGES','READ_MESSAGES','ATTACH_FILES','READ_MESSAGE_HISTORY']
  }]
})
}) }
...
client['on']('message', async (message) => {
  ...
  var channel = message
.guild.channels.get("773281277414473739");//حط ايدي الروم الي تجيه الرسائل انو حدا قدم للبارتنر وكدا 
  if(!channel)return;
  switch(command){
    case prefix+'partner':
      ...
      savePartner();
        ...
        savePartner();
        message.channel.send("**Success set the Partner role to "+getrole.name+"**");//3Mo_Steve is the best & n3k4a is one
      }//3Mo_Steve is the best & n3k4a is one
      break;//3Mo_Steve is the best & n3k4a is one
  }//3Mo_Steve is the best & n3k4a is one
})//3Mo_Steve is the best & n3k4a is one

function savePartner(){//3Mo_Steve is the best & n3k4a is one
	fs.writeFile("./partners.json", JSON.stringify(partners), function (err) {//3Mo_Steve is the best & n3k4a is one
        if(err) throw err;//3Mo_Steve is the best & n3k4a is one
    })//3Mo_Steve is the best & n3k4a is one
}//3Mo_Steve is the best & n3k4a is one
...
client.on("message", message => {
  ...
  if (!message.content.startsWith( "ناحو" , "هتوحشنا")) return;
  ...
  if (command == "ناحو" , "هتوحشنا") {
    ...
    let banforchannel = new Discord.RichEmbed()
          .setAuthor(user.username , user.avatarURL) 
          .setDescription(`**has been successfully banned**`)  
          .setColor('RANDOM')
          .setImage("https://cdn.discordapp.com/attachments/396964573007052800/535898524857008149/ban14CUT.gif")
        message.channel.send(banforchannel)
...
client.on ('message', async (toxicc) => {
  ...
  if (toxicc.content.startsWith (prefix + 'avatar')) {
    var user = toxicc.guild.member (toxicc.mentions.members.first() || toxicc.author);
      if (!toxicc.member.hasPermission("ADMINISTRATOR"))
      return toxicc.channel.send(
    ...
    .setFooter(`Justice Gaming Community` , toxicc.client.user.avatarURL)
    toxicc.channel.send(embed);
  }
})
//By 3Mo_Steve || Toxic Codes
  ...
  if (message.content.startsWith(prefix + "setby")) {
    ...
    client.on("guildMemberAdd", member => {
      ...
      var embed = new Discord.RichEmbed()
        ...
        .setDescription(`**مع السلامه  للعايز يمشي  ✋** `)
        .addField("👤   تبقي", `**[ ${member.guild.memberCount} ]**`, true)
        .setColor("RANDOM")
        .setFooter(`نتمنى لكم الاستمتاع`);

      var channel = member.guild.channels.find(gg => gg.name === "═│⚠ᴊᴜꜱᴛɪᴄᴇ-ʟᴇᴀᴠᴇ"); //// تعديل اساسي
      if (!channel) return;
